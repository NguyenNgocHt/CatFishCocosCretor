...
@ccclass('cf_WaitingProgress')
export class cf_WaitingProgress {
    private _isShowing: boolean = false;
    private _nodeWaiting: Node = null;
    private tweenWaitingRF!: Tween<Node>;

    static _instance: cf_WaitingProgress;

    static get instance() {
        if (!this._instance) {
            this._instance = new cf_WaitingProgress();
        }
    ...
    init() {
        ...
        let prefabWaiting = VDScreenManager.instance.assetBundle.get(cf_Path.WATING_PROGRESS, Prefab)!;
